...
public class FloodFill {
	...
	public ArrayList<CaveCell> landCells = new ArrayList<CaveCell>();
	...
	public void iterativeFillLand(int initialX, int initialY) {
		Queue<Point> points = new LinkedList<Point>();
		points.add(new Point(initialX, initialY));
		landCells = new ArrayList<CaveCell>();
		if(initialX<0)
			return;
		while (!points.isEmpty()) {
			Point currentPoint = points.remove();
			...
			if(currentTile instanceof GrassEntity  && currentTile.visited==0)
			{
				cell = new CaveCell(x, y+1);
				landCells.add(cell);
				currentTile.visited = 1;//true;
